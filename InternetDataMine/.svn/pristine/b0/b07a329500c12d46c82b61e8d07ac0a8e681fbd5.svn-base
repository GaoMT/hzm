...
function bldStr(ctlType, id, para, height)
{
 ...
 for(var i=0; i<arrScript.length; i++) {
	...
	var index = src.indexOf('dynaloadNG.js');		//假设dynaload.js文件名没变
	if(index >= 0) {
...
function insertGantt(id, para)		{ document.write( bldStr("Gantt",		id, para, (arguments.length > 2) ? arguments[2]:"100%") )}
